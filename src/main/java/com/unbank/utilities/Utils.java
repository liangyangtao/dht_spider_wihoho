...
public class Utils {
	...
	public static List<Node> decodeNodes(String encodeNodes)
			throws UnknownHostException {
		List<Node> result = new ArrayList<>();
		if (Strings.isNullOrEmpty(encodeNodes)) {
			return result;
		}
System.out.println(encodeNodes.length());
...
System.out.println(size % 26);
		...
		for (int i = 0; i < size; i += 26) {
			...
			String ip = InetAddress.getByAddress(currentNodeIp)
					.getHostAddress();

			Node n = new Node(
					new String(currentNodeId, Bencode.DEFAULT_CHARSET), ip,
					port);
